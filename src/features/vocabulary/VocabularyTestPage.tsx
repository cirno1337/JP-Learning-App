...
export function VocabularyTestPage() {
  ...
  const [queue, setQueue] = useState<{ vocab: VocabularyEntry; direction: Direction }[]>([]);
  ...
  const available = useMemo(
    () => dataset.vocabulary.filter((v) => lessonFilter === "all" || v.lessonIds.includes(lessonFilter)),
    [lessonFilter],
  );

  function startTest() {
    const picked = shuffle(available).slice(0, Math.min(count, available.length));
    const withDirection = picked.map((vocab) => ({
      vocab,
      direction:
        directionSetting === "mixed" ? (Math.random() < 0.5 ? "jp-to-en" : "en-to-jp") : directionSetting,
    })) as { vocab: VocabularyEntry; direction: Direction }[];
  ...
  function acceptedAnswersFor(vocab: VocabularyEntry, direction: Direction): string[] {
    if (direction === "jp-to-en") return meaningsToAcceptedAnswers(vocab.meanings, language);
    return vocab.kanji ? [vocab.kanji, vocab.kana] : [vocab.kana];
  }

  function submit() {
    if (!current || phase !== "question") return;
    const accepted = acceptedAnswersFor(current.vocab, current.direction);
    const isCorrect = checkAnswer(answer, accepted, current.direction === "jp-to-en" ? "english" : "japanese");
    finish(isCorrect ? "auto-correct" : "wrong");
  }

  function finish(outcome: AnswerOutcome) {
    if (!current) return;
    recordAttempt({ itemId: current.vocab.id, category: "vocabulary", outcome, direction: current.direction });
    setResult({ outcome });
  ...
  function acceptCloseEnough() {
    ...
    recordAttempt({ itemId: current.vocab.id, category: "vocabulary", outcome: "self-accepted", direction: current.direction });
  ...
  if (dataset.vocabulary.length === 0) {
    return (
      <div>
        <PageHeader title={t("nav.vocabularyTest")} />
        <EmptyState />
      </div>
    );
  }
  ...
  if (phase === "config") {
    return (
      <div>
        ...
        <p style={{ color: "var(--color-text-muted)" }}>{available.length} words available with this filter.</p>
  ...
  return (
    <div>
      <PageHeader title={t("nav.vocabularyTest")} description={`${qIndex + 1} / ${queue.length}`} />
      <div className="card test-question">
        {current.direction === "jp-to-en" ? (
          <div className="jp-text" style={{ marginBottom: "var(--space-2)" }}>
            <div className="test-question__prompt" style={{ fontSize: "2.5rem" }}>
              {current.vocab.kanji ?? current.vocab.kana}
            </div>
            {current.vocab.kanji && (
              <div className="test-question__reading">{current.vocab.kana}</div>
            )}
          </div>
        ) : (
          <div className="test-question__prompt" style={{ fontSize: "1.75rem" }}>
            {meaningsToAcceptedAnswers(current.vocab.meanings, language).join(" / ")}
          </div>
        )}
          ...
          <>
            ...
            <p>
              ...
              <strong className="jp-text">
                {current.direction === "jp-to-en"
                  ? localizedText(current.vocab.meanings, language)
                  : `${current.vocab.kanji ?? current.vocab.kana}${current.vocab.kanji ? ` (${current.vocab.kana})` : ""}`}
              </strong>
            </p>
            <div className="test-question__actions">
